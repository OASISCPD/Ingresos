...
import { InterfaceClient } from '../windows/NotAllowedDesktop';
...
export function ModalCreate(prop: propModal) {
    ...
    async function sendUpdate() {
        ...
        const updatedData = {
            ...data,
            apellido_nombre: `${prop.object.apellido} ${prop.object.nombre}`,
            id_foreign_key: prop.object.id_cliente.toString(),
            n_documento: prop.object.n_documento,
            fecha_hora_ingreso: formattedDateNow
        };

        const body = new URLSearchParams();
        body.append('apellido_nombre', updatedData.apellido_nombre);
        body.append('n_documento', updatedData.n_documento);
        ...
        body.append('fecha_hora_ingreso', updatedData.fecha_hora_ingreso);
        ...
        try {
            const response = await fetch(`${baseUrl}/insertar_no_permitido`, requestOptions);
            if (!response.ok) {
            ...
            if (response.status === 200) {
                ...
                prop.closeModal()
                await SwalMobile('error', 'Este cliente ya fue cargado anteriormente, si quieres editarlo, ve al registro y editalo desde ahi ', 3000);
                window.location.reload();
            }
        } catch (error) {
            console.error(error);
        ...
        <section className="fixed top-0 left-0 w-full h-full flex items-center justify-center  z-500">
            <div className="rounded-xl shadow-xl max-w-md w-full mx-4 md:mx-auto ">
                <div className=" bg-backgroundModalGray  rounded-lg shadow min-h-40 ">
                    ...
                    <p className='text-center my-2 text-white text-2xl'>Crear no permitido </p>
                    {/*   {client && ( */}
                    <div style={{ maxHeight: maxHeight, overflowY: 'auto', overflowX: "auto" }} className="px-4 py-2 scrollbar   text-gray-200">
                        <div /* onSubmit={handleSubmit} */>
                            <div className="my-2">
                                <label className="block mb-2 text-sm  ">Características físicas *</label>
                                <input type="input"  {...register('caracteristicas_fisicas')} id="caracteristicas_fisicas" defaultValue={''} onChange={handleInputChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="completa este campo" required />
                            </div>
                            <div className="my-2">
                                <label className="block mb-2 text-sm  ">Motivo</label>
                                <input type="input"{...register('motivo')} id="motivo" defaultValue={''} onChange={handleInputChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="completa este campo" required />
                            </div>
                            <div className="my-2">
                            ...
                            <div className="my-2">
                                ...
                                <select onChange={(e) => {
                                    ...
                                    <option value={"other"} selected disabled={true}>Elige una opcion</option>
