...
export function ManualEntryCountMobileTablet({ domain }: Domain) {
    ...
    async function occupation() {
        ...
        if (res.total_dataaforo > 600) {
            ...
            Toast.fire({
                icon: "warning",
                title: "¡ATENCIÓN! El aforo de la sala ha alcanzado las 600 personas. Por favor, verifique si este número es correcto."
            });
        }
        ...
        <section>
            <div className="pb-8 pt-4 sm:pt-0 text-white text-xs sm:text-base">
                <div className="">
                    ...
                    <button className="flex  text-2xl sm:text-6xl items-center justify-center mx-auto text-greenText text-center  bg-black bg-opacity-70 border border-backgroundBlack py-3 px-8 w-full focus:outline-none rounded-xl shadow-xl ">{aforoOccupation?.total_dataaforo}</button>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-1  text-base sm:text-2xl py-2 gap-4">
                    <div className="grid grid-cols-2 gap-4">
                        <button onClick={() => incomeValue(1)}
                            className="animate-buttonClick flex items-center justify-center mx-auto w-full bg-blueColor border-0 py-3 px-8 focus:outline-none rounded-xl shadow-xl    duration-300"
                        >+1
                        </button>
                        <button onClick={() => egressValue(1)}
                            className="animate-buttonClick flex items-center justify-center mx-auto w-full bg-buttonRed border-0 py-3 px-8 focus:outline-none rounded-xl shadow-xl    duration-300"
                        >-1
                        </button>
                    </div>
                </div>
                <div className="grid grid-cols-2 text-base sm:text-2xl py-2 gap-4">
                    <button onClick={() => incomeValue(5)}
                        className="animate-buttonClick flex items-center justify-center mx-auto bg-blueColor border-0 py-3 px-8 w-full focus:outline-none rounded-xl shadow-xl    duration-300"
                    >+5</button>
                    <button onClick={() => egressValue(5)}
                        className="animate-buttonClick flex items-center justify-center mx-auto bg-buttonRed border-0 py-3 px-8 w-full focus:outline-none rounded-xl shadow-xl    duration-300"
                    >-5</button>
                </div>
                <div className="grid grid-cols-2 text-base sm:text-2xl py-2 gap-4">
